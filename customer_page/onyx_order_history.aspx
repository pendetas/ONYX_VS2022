        ...
        <div class="onyx-account-layout">
            <aside class="onyx-account-sidebar">
                ...
                <nav class="onyx-account-nav" aria-label="Account navigation">
                    <a href="/customer_page/onyx_profile">
                    ...
                    <a class="is-active" href="/customer_page/onyx_order_history">
            ...
            <div class="onyx-account-main">
                <section class="onyx-account-section onyx-order-section" aria-labelledby="order-history-title" aria-describedby="order-history-description">
                    <header class="onyx-order-header">
                        <div>
                            <h2 id="order-history-title" class="onyx-section-title">Order history</h2>
                            <p id="order-history-description" class="onyx-page-lede">Track recent ONYX purchases, payment status, and receipt downloads from one focused page.</p>
                        </div>
                        <nav class="onyx-order-filters" aria-label="Order status filters">
                    ...
                    <asp:Repeater ID="rptRecentOrders" runat="server" OnItemCommand="rptRecentOrders_ItemCommand">
                        <HeaderTemplate>
                            ...
                            <div class="onyx-order-list" role="list">
                        </HeaderTemplate>
                        <ItemTemplate>
                            <article class="onyx-order-card" role="listitem" aria-label='<%# "Order " + Eval("Id") %>'>
                                <div class="onyx-order-identity">
                                    <span class="onyx-order-id">Order #<%# Eval("Id") %></span>
                                    <time class="onyx-order-date"><%# FormatOrderDate(Eval("OrderedAt")) %></time>
                                </div>
                                <p class="onyx-order-summary"><%# GetOrderSummary(Container.DataItem) %></p>
                                <div class="onyx-order-state">
                                    <span class='<%# GetStatusClass(Eval("Status")) %>'><%# FormatStatus(Eval("Status")) %></span>
                                </div>
